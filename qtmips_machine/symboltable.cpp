...
SymbolTable::~SymbolTable() {
    map_value_to_symbol.clear(); // Does not own data.
    auto iter = map_name_to_symbol.begin();
    while (iter != map_name_to_symbol.end()) {
        const SymbolTableEntry *p_entry = iter.value();
        iter = map_name_to_symbol.erase(iter); // Advances iterator.
        delete p_entry;
    }
}
