    ...
    , replacement_policy(CachePolicy::get_policy_instance(config))
{
    ...
    dt.resize(config->associativity(), std::vector<CacheBlock>(config->set_count(), { .valid = false, .dirty = false, .data = std::vector<uint32_t>(config->block_size()) }));
}
...
void Cache::flush()
{
    ...
    for (auto assoc_index = cache_config.associativity(); assoc_index > 0; assoc_index--) {
        for (auto set_index = 0; set_index < cache_config.set_count(); set_index++) {
...
void Cache::reset()
{
    ...
    if (cache_config.enabled()) {
        for (auto assoc_index = 0; assoc_index < cache_config.associativity(); assoc_index++) {
            for (auto set_index = 0; set_index < cache_config.set_count(); set_index++) {
                emit cache_update(assoc_index, set_index, 0, false, false, 0, nullptr, false);
            }
        }
    ...
    size_t size) const
{
    ...
    for (auto assoc_index = 0; assoc_index < cache_config.associativity(); assoc_index++) {
        if (dt[assoc_index][loc.set].valid && dt[assoc_index][loc.set].tag == loc.tag) {
            memory_copy(destination, (byte*)&dt[assoc_index][loc.set].data[loc.col] + loc.bytes, size);
            return;
        }
    }
    ...
    AccessType access_type) const
{
    ...
    if (cd.valid) {
        ...
        mem->read(calc_base_address(loc.tag, loc.set), cd.data.data(),
    ...
    size_t set_index) const
{
    ...
    if (cd.dirty && cache_config.write_policy() == CacheConfig::WP_BACK) {
        mem->write(
            calc_base_address(cd.tag, set_index),
            cd.data.data(),
            cache_config.block_size() * BLOCK_ITEM_SIZE);
...
{
    return Address(
        (tag * cache_config.set_count() + set_index)
        * cache_config.block_size() * BLOCK_ITEM_SIZE);
}
...
uint32_t Cache::get_stall_count() const
{
    ...
    if (access_pen_b != 0) {
        st_cycles -= burst_reads * (access_pen_r - access_pen_b) + burst_writes * (access_pen_w - access_pen_b);
    }
    return st_cycles;
}
...
double Cache::get_speed_improvement() const
{
    ...
    if (access_pen_b != 0) {
        mem_access_time -= burst_reads * (access_pen_r - access_pen_b) + burst_writes * (access_pen_w - access_pen_b);
    }
