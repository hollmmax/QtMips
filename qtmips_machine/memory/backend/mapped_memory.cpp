...
// TODO this file was created by copying memory.cpp, and wasn't fully cleaned afterwards
...
MappedMemory::MappedMemory(QFileDevice* file, size_t length, size_t offset) : file(file), len(length)  {
    this->dt = file->map(offset, length);
    // TODO mapping can fail and should be reported upwards
    // since this class should be instantiated only in the mmap/mmap2 syscall,
    // the simplest way to handle error would be to throw an exception here,
    // catch it there, and return from syscall as if it failed on real hardware.
}

MappedMemory::~MappedMemory() {
    file->unmap(this->dt);
    // TODO according to Qt documentation, this call can fail.
    // The only situation I can think of is if we were to call unmap with the same
    // `ptr` before, or we give a `ptr` not previously returned from map.
    // I'm therefore I am content leaving it like this, but someone else should
    // check my reasoning and remove the TODO label.
}

bool MappedMemory::write(
    const void *source,
    Offset offset,
    size_t count
) {
...
size_t MappedMemory::length() const {
    return this->len;
}

const uchar* MappedMemory::data() const {
    return this->dt;
}
...
// TODO I think none of the following is relevant for `MappedMemory`,
// but I don't feel confident in my decision making at 3AM...
/*
//////////////////////////////////////////////////////////////////////////////
