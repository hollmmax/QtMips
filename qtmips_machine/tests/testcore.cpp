...
void MachineTests::singlecore_regs_data() { core_regs_data(); }

void MachineTests::pipecore_regs_data() { core_regs_data(); }
...
void MachineTests::singlecore_regs() {
    ...
    TrivialMMU mem_used_frontend(&mem);
...
void MachineTests::pipecore_regs() {
    ...
    Memory mem_used(mem);
    TrivialMMU mem_used_frontend(&mem_used);
...
void MachineTests::singlecore_jmp_data() { core_jmp_data(); }

void MachineTests::pipecore_jmp_data() { core_jmp_data(); }
...
void MachineTests::singlecore_mem_data() { core_mem_data(); }

void MachineTests::pipecore_mem_data() { core_mem_data(); }
...
void MachineTests::singlecore_mem() {
    ...
    TrivialMMU mem_init_frontend(&mem_init);
...
static void core_alu_forward_data() {
    ...
    // Test multiplication and division
    {
        ...
        regs_res.write_gp(
            20, (uint32_t)((std::int32_t)val_b / (std::int32_t)val_a));
        regs_res.write_gp(
            21, (uint32_t)((std::int32_t)val_b % (std::int32_t)val_a));
...
void MachineTests::singlecore_alu_forward_data() { core_alu_forward_data(); }

void MachineTests::pipecore_alu_forward_data() { core_alu_forward_data(); }

void MachineTests::pipecorestall_alu_forward_data() { core_alu_forward_data(); }
...
static void run_code_fragment(
    ...
    for (int k = 10000; k; k--) {
        core.step(); // Single step should be enought as this is risc without
                     // pipeline
        if (reg_init.read_pc() == reg_res.read_pc() && k > 6) // reached end of
                                                              // the code
                                                              // fragment
    ...
    for (int i = 0; i < 32; ++i) {
        if (reg_init.read_gp(i) != reg_res.read_gp(i)) {
            printf(
                "DIFF reg[%d] %lx %lx\n", i, reg_init.read_gp(i).as_u64(),
                reg_res.read_gp(i).as_u64());
        }
    }
...
void MachineTests::singlecore_memory_tests_data() { core_memory_tests_data(); }

void MachineTests::pipecore_nc_memory_tests_data() { core_memory_tests_data(); }
...
void MachineTests::pipecore_wb_memory_tests_data() { core_memory_tests_data(); }
