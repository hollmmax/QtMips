...
static inline void alu_write_hi_lo_64bit(Registers *regs, std::uint64_t val)
{
    ...
    regs->write_hi_lo(true,  (std::uint32_t)(val >> 32U));
}


RegisterValue machine::alu_operate(enum AluOp operation, RegisterValue s,
    ...
    switch(operation) {
        ...
        case ALU_OP_MULT:
            ...
            return 0x0;
        case ALU_OP_MULTU:
            u64_val = (std::uint64_t) s.as_u32() * t.as_u32();
        ...
        case ALU_OP_DIV:
            ...
            regs->write_hi_lo(false, (std::uint32_t)(s.as_i32() / t.as_i32()));
            regs->write_hi_lo(true,  (std::uint32_t)(s.as_i32() % t.as_i32()));
            return 0x0;
        case ALU_OP_DIVU:
            if (t.as_u32() == 0) {
                regs->write_hi_lo(false, 0);
        ...
        case ALU_OP_SLTU:
            return (s.as_u32() < t.as_u32()) ? 1 : 0;
        case ALU_OP_MUL:
            return (std::uint32_t)(s.as_i32() * t.as_i32());
        case ALU_OP_MADD:
        ...
        case ALU_OP_LUI:
            return t.as_u32() << 16U;
        case ALU_OP_WSBH:
            return ((t.as_u32() << 8U) & 0xff00ff00) | ((t.as_u32() >> 8U) & 0x00ff00ffU);
        case ALU_OP_SEB:
            return (uint32_t)(int32_t)(int8_t)t.as_u32();
        case ALU_OP_SEH:
            return (uint32_t)(int32_t)(int16_t)t.as_u32();
        case ALU_OP_EXT:
            return (s.as_u32() >> sa) & ((1 << (sz + 1)) - 1);
        case ALU_OP_INS:
            u32_val = (1 << (sz + 1)) - 1;
            return ((s.as_u32() & u32_val) << sa) | (t.as_u32() & ~(u32_val << sa));
        case ALU_OP_CLZ:
            return alu_op_clz(s.as_u32());
        case ALU_OP_CLO:
           return alu_op_clz(~s.as_u32());
