...
QtMipsMachine::QtMipsMachine(
    ...
    connect(
        ser_port, SIGNAL(signal_interrupt(uint, bool)), this,
        SIGNAL(set_interrupt_signal(uint, bool)));

    perip_spi_led = new PeripSpiLed();
    addressapce_insert_range(
...
const MachineConfig& QtMipsMachine::config() { return mcnf; }
...
const Registers* QtMipsMachine::registers() { return regs; }

const Cop0State* QtMipsMachine::cop0state() { return cop0st; }

const Memory* QtMipsMachine::memory() { return mem; }

Memory* QtMipsMachine::memory_rw() { return mem; }

const Cache* QtMipsMachine::cache_program() { return cch_program; }

const Cache* QtMipsMachine::cache_data() { return cch_data; }
...
const MMU* QtMipsMachine::physical_address_space() { return physaddrspace; }

MMU* QtMipsMachine::physical_address_space_rw() { return physaddrspace; }

SerialPort* QtMipsMachine::serial_port() { return ser_port; }

PeripSpiLed* QtMipsMachine::peripheral_spi_led() { return perip_spi_led; }
...
const Core* QtMipsMachine::core() { return cr; }
...
enum QtMipsMachine::Status QtMipsMachine::status() { return stat; }

bool QtMipsMachine::exited() { return stat == ST_EXIT || stat == ST_TRAPPED; }
...
void QtMipsMachine::step() { step_internal(true); }

void QtMipsMachine::step_timer() { step_internal(); }
