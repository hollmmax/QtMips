...
ProgramLoader::ProgramLoader(const char* file) {
    ...
    // Initialize elf
    if (!(this->elf = elf_begin(this->fd, ELF_C_READ, nullptr))) {
...
ProgramLoader::ProgramLoader(const QString& file)
    : ProgramLoader(file.toStdString().c_str()) {}
...
Address ProgramLoader::get_executable_entry() { return executable_entry; }
