...
ProgramLoader::ProgramLoader(const QString &file) : elf_file(file) {
    ...
    if (elf_class == ELFCLASS32) {
        ...
        for (unsigned i = 0; i < n_secs; i++) {
            if (sections_headers.arch32[i].p_type != PT_LOAD) {
                continue;
            }
        ...
        for (unsigned i = 0; i < this->n_secs; i++) {
            if (sections_headers.arch64[i].p_type != PT_LOAD) {
                continue;
            }
    ...
    if (architecture_type == ARCH32) {
        for (size_t phdrs_i : this->indexes_of_load_sections) {
            uint32_t base_address
                = this->sections_headers.arch32[phdrs_i].p_vaddr;
            char *f = elf_rawfile(this->elf, nullptr);
            for (unsigned y = 0;
                 y < this->sections_headers.arch32[phdrs_i].p_filesz; y++) {
                const auto buffer = (uint8_t)
                    f[this->sections_headers.arch32[phdrs_i].p_offset + y];
        ...
        for (size_t phdrs_i : this->indexes_of_load_sections) {
            uint32_t base_address
                = this->sections_headers.arch64[phdrs_i].p_vaddr;
            char *f = elf_rawfile(this->elf, nullptr);
            for (unsigned y = 0;
                 y < this->sections_headers.arch64[phdrs_i].p_filesz; y++) {
                const auto buffer = (uint8_t)
                    f[this->sections_headers.arch64[phdrs_i].p_offset + y];
                memory_write_u8(mem, base_address + y, buffer);
            }
        }
    }
