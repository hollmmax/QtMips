...
protected:
    ...
    struct dtDecode {
        ...
        bool regwrite;             // If output should be written back to register (which
                                   // one depends on regd)
        bool alu_req_rs1;          // requires rs value for ALU
        bool alu_req_rs2;          // requires rt value for ALU or SW
        bool bjr_req_rs1;          // requires rs for beq, bne, blez, bgtz, jr nad jalr
        bool bjr_req_rs2;          // requires rt for beq, bne
        ...
        uint8_t num_rs1;           // Number of the register s1
        uint8_t num_rs2;           // Number of the register s2
        uint8_t num_rd;            // Number of the register d
        RegisterValue val_rs1;     // Value from register rs1
        RegisterValue val_rs2;     // Value from register rs2
