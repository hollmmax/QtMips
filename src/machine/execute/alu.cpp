...
int64_t
alu64_operate(AluOp op, bool modified, RegisterValue a, RegisterValue b) {
...
int32_t
alu32_operate(AluOp op, bool modified, RegisterValue a, RegisterValue b) {
    ...
    case AluOp::XOR:
        ...
        // Most compilers should calculate SRA correctly, but it is UB.
    case AluOp::SR:
        return (modified) ? (a.as_i64() >> (_b & SHIFT_MASK))
                          : (_a >> (_b & SHIFT_MASK));
    case AluOp::OR: return _a | _b;
    case AluOp::AND: return _a & _b;
    default:
...
int32_t mul32_operate(MulOp op, RegisterValue a, RegisterValue b) {
    ...
    case MulOp::MULH:
        return ((uint64_t)a.as_i32() * (uint64_t)b.as_i32()) >> 32;
    case MulOp::MULHSU:
        return ((uint64_t)a.as_i32() * (uint64_t)b.as_u32()) >> 32;
    case MulOp::MULHU:
        return ((uint64_t)a.as_u32() * (uint64_t)b.as_u32()) >> 32;
    case MulOp::DIV:
        if (b.as_i32() == 0) {
            return -1; // Division by zero is defined.
