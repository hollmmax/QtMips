...
void TestAlu::test_alu64_operate_data() {
    ...
    QTest::addRow("ADD") << AluOp::ADD << false
                         << int64_t(0xFFFFFFFFFFFFFFFFULL) << int64_t(1)
                         << int64_t(0);
    QTest::addRow("ADD") << AluOp::ADD << false << int64_t(123)
                         << int64_t(123000) << int64_t(123123);
    QTest::addRow("SUB") << AluOp::ADD << true << int64_t(123123)
                         << int64_t(123000) << int64_t(123);
    QTest::addRow("SLT") << AluOp::SLT << false << int64_t(123123)
                         << int64_t(123000) << int64_t(0);
    QTest::addRow("SLT") << AluOp::SLT << false << int64_t(-123123)
                         << int64_t(123000) << int64_t(1);
    QTest::addRow("SLTU") << AluOp::SLTU << false << int64_t(-123123)
                          << int64_t(123000) << int64_t(0);
    QTest::addRow("SLTU") << AluOp::SLTU << false << int64_t(123123)
                          << int64_t(123000000) << int64_t(1);

    QTest::addRow("XOR") << AluOp::XOR << false
...
void TestAlu::test_alu64_operate() {
    ...
    QCOMPARE(
        alu_combined_operate(
            { .alu_op = op }, AluComponent::ALU, false, modified, operand_a,
            operand_b),
...
void TestAlu::test_alu32_operate_data() {
    ...
    QTest::addRow("ADD") << AluOp::ADD << false << int32_t(0xFFFFFFFF)
                         << int32_t(1) << int32_t(0);
    QTest::addRow("ADD") << AluOp::ADD << false << int32_t(123)
                         << int32_t(123000) << int32_t(123123);
    QTest::addRow("SUB") << AluOp::ADD << true << int32_t(123123)
                         << int32_t(123000) << int32_t(123);
    QTest::addRow("SLT") << AluOp::SLT << false << int32_t(123123)
                         << int32_t(123000) << int32_t(0);
    QTest::addRow("SLT") << AluOp::SLT << false << int32_t(-123123)
                         << int32_t(123000) << int32_t(1);
    QTest::addRow("SLTU") << AluOp::SLTU << false << int32_t(-123123)
                          << int32_t(123000) << int32_t(0);
    QTest::addRow("SLTU") << AluOp::SLTU << false << int32_t(123123)
                          << int32_t(123000000) << int32_t(1);
    QTest::addRow("XOR") << AluOp::XOR << false << int32_t(0xFFFF0000)
                         << int32_t(0xFFFFFFFF) << int32_t(0x0000FFFF);
    QTest::addRow("OR") << AluOp::OR << false << int32_t(0xFFFF0000)
                        << int32_t(0xFFFFFFFF) << int32_t(0xFFFFFFFF);
    QTest::addRow("AND") << AluOp::AND << false << int32_t(0xFFFF0000)
                         << int32_t(0xFFFFFFFF) << int32_t(0xFFFF0000);
    QTest::addRow("SRL") << AluOp::SR << false << int32_t(0xFFFF0000)
                         << int32_t(0xFFFF0010) << int32_t(0x0000FFFF);
    QTest::addRow("SRA") << AluOp::SR << true << int32_t(0xFFFF0000)
                         << int32_t(0xFFFF0010) << int32_t(0xFFFFFFFF);
}
...
void TestAlu::test_alu32_operate() {
    ...
    QCOMPARE(
        alu_combined_operate(
            { .alu_op = op }, AluComponent::ALU, true, modified, operand_a,
            operand_b),
        RegisterValue(result));
}
...
void TestAlu::test_mul64_operate_data() {
    ...
    for (auto input : inputs) {
        ...
        QTest::addRow("MULHU")
            << MulOp::MULHU << a << b << int64_t(mulhu64(a, b));
        QTest::addRow("MULHSU")
            << MulOp::MULHSU << a << b << int64_t(mulhsu64(a, b));
        QTest::addRow("DIV") << MulOp::DIV << a << b << a / b;
        QTest::addRow("DIVU")
            << MulOp::DIVU << a << b << int64_t(uint64_t(a) / uint64_t(b));
        QTest::addRow("REM") << MulOp::REM << a << b << (a % b);
        QTest::addRow("REMU")
            << MulOp::REMU << a << b << int64_t(uint64_t(a) % uint64_t(b));
    }
...
void TestAlu::test_mul64_operate() {
    ...
    QCOMPARE(
        alu_combined_operate(
            (AluCombinedOp) { .mul_op = op }, AluComponent::MUL, false, false,
            operand_a, operand_b),
        RegisterValue(result));
}
...
void TestAlu::test_mul32_operate_data() {
    ...
    for (auto input : inputs) {
        ...
        QTest::addRow("MULHSU")
            << MulOp::MULHSU << a << b << int32_t(mulh32(a, uint32_t(b)));
        QTest::addRow("DIV") << MulOp::DIV << a << b << a / b;
        QTest::addRow("DIVU")
            << MulOp::DIVU << a << b << int32_t(uint32_t(a) / uint32_t(b));
        QTest::addRow("REM") << MulOp::REM << a << b << (a % b);
        QTest::addRow("REMU")
            << MulOp::REMU << a << b << int32_t(uint32_t(a) % uint32_t(b));
    }
...
void TestAlu::test_mul32_operate() {
    ...
    QCOMPARE(
        alu_combined_operate(
            (AluCombinedOp) { .mul_op = op }, AluComponent::MUL, true, false,
            operand_a, operand_b),
        RegisterValue(result));
}
