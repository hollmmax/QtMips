...
void CoreViewScene::install_values_from_document<PCValue>(
    ...
    const unordered_map<QStringView, Lens<CoreState, Address>>
        &value_source_name_map,
    const CoreState &core_state);

CoreViewScene::CoreViewScene(
    machine::Machine *machine,
    const QString &core_svg_scheme_name)
    : SvgGraphicsScene() {
    Q_UNUSED(machine)
...
void CoreViewScene::install_hyperlink(svgscene::HyperlinkItem *element) const {
    ...
    try {
        ...
        DEBUG("Registered hyperlink to target %s",
...
void CoreViewScene::install_value(
    ...
    try {
        ...
        WARN(
            "Source for bool value not found (source: \"%s\").",
            qPrintable(source_name));
    }
}
...
void CoreViewScene::install_values_from_document<PCValue>(
    ...
    const CoreState &core_state) {
    for (SvgDomTree<QGraphicsItem> component_tree : document.getRoot().findAll(
             "data-component", PCValue::COMPONENT_NAME)) {
        SimpleTextItem *text_element
            = component_tree.find<SimpleTextItem>().getElement();
        QString source_name = component_tree.getAttrValueOr("data-source", "");
        ...
        try {
            ...
            HyperlinkItem *hyperlink
                = component_tree.find<HyperlinkItem>().getElement();
            connect(
                hyperlink, &HyperlinkItem::triggered, handler,
                &PCValue::clicked);
            connect(
                handler, &PCValue::jump_to_pc, this,
                &CoreViewScene::request_jump_to_program_counter);
            connect(
                hyperlink, &HyperlinkItem::triggered, this,
                &CoreViewScene::request_program_memory);
        } catch (std::out_of_range &e) {
            DEBUG("PC component without a hyperlink.");
        }
    }
}
