 ...
 * ```
 */
static const unordered_map<QString, void (::CoreViewScene::*)()>
    HYPERLINK_TARGETS {
...
using InstructionData
    = pair<const machine::Instruction &, const machine::Address &>;
...
const struct {
    const unordered_map<QStringView, Lens<CoreState, bool>> BOOL {
        { QStringLiteral("alu-zero"),
          LENS(CoreState, pipeline.execute.internal.alu_zero) },
        { QStringLiteral("decode-alu-src"),
          LENS(CoreState, pipeline.decode.result.alusrc) },
        { QStringLiteral("decode-branch"),
          LENS(CoreState, pipeline.decode.result.branch) },
        { QStringLiteral("decode-mem-read"),
          LENS(CoreState, pipeline.decode.result.memread) },
        { QStringLiteral("decode-mem-to-reg"),
          LENS(CoreState, pipeline.decode.result.memwrite) },
        { QStringLiteral("decode-mem-write"),
          LENS(CoreState, pipeline.decode.result.memwrite) },
        { QStringLiteral("decode-reg-write"),
          LENS(CoreState, pipeline.decode.result.regwrite) },
        { QStringLiteral("exec-alu-src"),
          LENS(CoreState, pipeline.execute.internal.alu_src) },
        { QStringLiteral("exec-branch"),
          LENS(CoreState, pipeline.execute.internal.branch) },
        { QStringLiteral("exec-mem-read"),
          LENS(CoreState, pipeline.execute.result.memread) },
        { QStringLiteral("exec-mem-to-reg"),
          LENS(CoreState, pipeline.execute.result.memread) },
        { QStringLiteral("exec-mem-write"),
          LENS(CoreState, pipeline.execute.result.memwrite) },
        { QStringLiteral("exec-reg-write"),
          LENS(CoreState, pipeline.execute.result.regwrite) },
        // TODO: NOT AVAILABLE IN MIPS CORE
        //        { QStringLiteral("exec-branch"),
        //          LENS(CoreState, pipeline.execute.inside.) },
        { QStringLiteral("mem-mem-read"),
          LENS(CoreState, pipeline.memory.internal.memread) },
        { QStringLiteral("mem-mem-to-reg"),
          LENS(CoreState, pipeline.memory.result.memtoreg) },
        { QStringLiteral("mem-mem-write"),
          LENS(CoreState, pipeline.memory.internal.memwrite) },
        { QStringLiteral("mem-reg-write"),
          LENS(CoreState, pipeline.memory.result.regwrite) },
        { QStringLiteral("mem-to-reg"),
          LENS(CoreState, pipeline.memory.result.memtoreg) },
        { QStringLiteral("wb-reg-write"),
          LENS(CoreState, pipeline.writeback.internal.regwrite) },
    };
    const unordered_map<QStringView, Lens<CoreState, RegisterValue>> REG {
        { QStringLiteral("alu-res"),
          LENS(CoreState, pipeline.execute.result.alu_val) },
        { QStringLiteral("alu-src1"),
          LENS(CoreState, pipeline.execute.internal.alu_src1) },
        { QStringLiteral("alu-src2"),
          LENS(CoreState, pipeline.execute.internal.alu_src2) },
        { QStringLiteral("decode-imm"),
          LENS(CoreState, pipeline.decode.result.immediate_val) },
        { QStringLiteral("exec-imm"),
          LENS(CoreState, pipeline.execute.internal.immediate) },
        { QStringLiteral("fetched-value"),
          LENS(CoreState, pipeline.fetch.internal.fetched_value) },
        { QStringLiteral("mem-write-val"),
          LENS(CoreState, pipeline.memory.internal.mem_write_val) },
        { QStringLiteral("mem-read-val"),
          LENS(CoreState, pipeline.memory.internal.mem_read_val) },
        { QStringLiteral("decode-rs1"),
          LENS(CoreState, pipeline.decode.result.val_rs) },
    ...
    const unordered_map<QStringView, Lens<CoreState, uint8_t>> REG_ID {
        { QStringLiteral("decode-rd"),
          LENS(CoreState, pipeline.decode.result.num_rd) },
        { QStringLiteral("exec-rd"),
          LENS(CoreState, pipeline.execute.result.num_rd) },
        { QStringLiteral("mem-rd"),
          LENS(CoreState, pipeline.memory.result.num_rd) },
        { QStringLiteral("wb-rd"),
          LENS(CoreState, pipeline.writeback.internal.num_rd) },
        { QStringLiteral("rs1"),
    ...
    const unordered_map<QStringView, Lens<CoreState, Address>> PC {
        { QStringLiteral("fetch-pc"),
          LENS(CoreState, pipeline.fetch.result.inst_addr) },
    };

#define MULTITEXT_LENS(INDEX, TABLE)                                           \
    [](const CoreState &base)                                                  \
        -> std::pair<const unsigned &, const vector<QString> &> {              \
        return { base.INDEX, TABLE };                                          \
    }

    const unordered_map<
        QStringView,
        LensPair<CoreState, unsigned, vector<QString>>>
        MULTI_TEXT {
            { QStringLiteral("fetch-exception"),
              MULTITEXT_LENS(
                  pipeline.fetch.internal.excause_num,
                  EXCEPTION_NAME_TABLE) },
            { QStringLiteral("decode-exception"),
              MULTITEXT_LENS(
                  pipeline.decode.internal.excause_num,
                  EXCEPTION_NAME_TABLE) },
            { QStringLiteral("execute-exception"),
              MULTITEXT_LENS(
                  pipeline.execute.internal.excause_num,
                  EXCEPTION_NAME_TABLE) },
            { QStringLiteral("memory-exception"),
              MULTITEXT_LENS(
                  pipeline.memory.internal.excause_num,
                  EXCEPTION_NAME_TABLE) },
            { QStringLiteral("hazard"), MULTITEXT_LENS(
                                            pipeline.execute.internal.stall_status,
                                            STALL_TEXT_TABLE) },
        };

    const unordered_map<QStringView, LensPair<CoreState, Instruction, Address>>
        INSTRUCTION {
            { QStringLiteral("fetch"),
              LENS_PAIR(
                  CoreState,
                  pipeline.fetch.result.inst,
                  pipeline.fetch.result.inst_addr) },
            { QStringLiteral("decode"),
              LENS_PAIR(
                  CoreState,
                  pipeline.decode.result.inst,
                  pipeline.decode.result.inst_addr) },
            { QStringLiteral("exec"),
              LENS_PAIR(
                  CoreState,
                  pipeline.execute.result.inst,
                  pipeline.execute.result.inst_addr) },
            { QStringLiteral("mem"),
              LENS_PAIR(
                  CoreState,
                  pipeline.memory.result.inst,
                  pipeline.memory.result.inst_addr) },
            { QStringLiteral("wb"),
              { LENS_PAIR(
                  CoreState,
