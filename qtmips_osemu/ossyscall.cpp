...
#include <cerrno>
#include "../qtmips_machine/memory/backend/mapped_memory.h"
#include <cstdio>
#include <fcntl.h>
#include <sys/stat.h>
#include <unistd.h>
...
int OsSyscallExceptionHandler::do_sys_mmap2(
    ...
    return 0;
    // auto file = file_objects[fd];
    // auto data = file->map(offset, lenght);
    // if (data == nullptr) return -1; // TODO: better error return
    // BackendMemory* chunk = new MappedMemory(data, lenght, file);
    // core->get_mem_data(); // TODO: insert mapping, but memory isn't ready for it yet.
}
